...
class AethernetStasisServer:
    ...
    async def start_and_wait(self) -> None:
        ...
        event_loop.add_signal_handler(signal.SIGTERM, self._stop_event.set)
        # noinspection PyTypeChecker
        event_loop.add_signal_handler(signal.SIGINT, self._stop_event.set)
    ...
    async def _stream_loop(self) -> None:
        while self.connected:
            try:
                ...
                self._logger.info(f"[loop] mode={mode.mode}")
                ...
                if mode.mode == "on_request":
                    ...
                    self._logger.info("[loop] waiting for interval/mode_change")
                    mode_wait = asyncio.create_task(self._mode_changed_event.wait())
                    try:
                        await asyncio.wait_for(asyncio.shield(mode_wait), timeout=interval_s)
                        self._mode_changed_event.clear()
                    except asyncio.TimeoutError:
                        self._logger.info("[loop] timeout -> sending screenshot")
                        await self._send_screenshot()
                        self._logger.info("[loop] screenshot sent, looping back")
                    finally:
                        mode_wait.cancel()
            except asyncio.CancelledError:
                return
